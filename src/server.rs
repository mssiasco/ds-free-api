//! HTTP server layer — thin routing shell exposing OpenAIAdapter and AnthropicCompat as HTTP interfaces
//!
...
fn build_router(state: AppState, cors_origins: Vec<String>) -> Router {
    ...
    // Static file serving: /admin → web/dist/
    // Read from filesystem first (dev mode), fall back to compile-time embedded assets (release binary)
...
/// Compile-time embed the web/dist/ directory; release binaries can serve the admin panel without extra files
#[derive(rust_embed::Embed)]
#[folder = "web/dist/"]
struct WebAssets;

/// Compile-time embedded asset fallback: only handles /admin and /admin/* paths, returns 404 for everything else
...
/// API Key authentication middleware (validates Bearer token from api_keys.json)
